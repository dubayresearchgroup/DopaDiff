#!/bin/bash

###############################
adsorbate=`cut -d'/' -f5 <<< "$(pwd)"`
surface=`cut -d'/' -f6 <<< "$(pwd)"`
system="${adsorbate}_${surface}"
surface=`cut -d'_' -f1 <<< ${surface}`

if [[ ${surface} == *"CNT"* ]]; then
    surface_category="CNT"
elif [[ ${surface} == "graphene" ]]; then 
    surface_category="G"
elif [[ ${surface} == *"BNNT"* ]]; then
    surface_category="BNNT"
elif [[ ${surface} == *"boronnitride"* ]]; then 
    surface_category="BN"
elif [[ ${surface} == *"groove"* ]]; then
    surface_category="groove"
elif [[ ${surface} == "graphene3" ]]; then
    surface_category="graphene3"
else exit 1
fi
###############################

# 1. Initialization.
cd nvt
rm vacf_integration.py tex_stats.tex

# 2， MSD & VACF computation.
cp $HOME/Desktop/analysis/fitting/vacf_integration.py .
python vacf_integration.py

# 3. File cleanup.
rm vacf_integration.py
if [ ! -d $HOME/Desktop/images ]; then
    mkdir $HOME/Desktop/images
fi
if [ ! -d $HOME/Desktop/images/vacf_integration ]; then
    mkdir $HOME/Desktop/images/vacf_integration
fi
mv tex_stats.tex $HOME/Desktop/images/vacf_integration/${system}.tex
cd ..
rm 13_vacf_integration.sh

