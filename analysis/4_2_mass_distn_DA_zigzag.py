...
for i in range(len(systems)):
    ...
    if ('graphene' in systems[i]) or ('boronnitride' in systems[i]): 
        ...
        df = pd.read_csv(filename, delimiter = ' ', \
                         ...
                         header = None)
        ax1.plot(df['bins'], df['avgVelocity'], linestyle=linestyle[i], \
                 c=cm(cmap_list[i]))
        ax1.plot(-df['bins'], df['avgVelocity'], linestyle=linestyle[i], \
                 c=cm(cmap_list[i]))
        ax2.plot(df['bins'], df['avgWaterMass']/2, linestyle=linestyle[i], \
                 c=cm(cmap_list[i]))
        ax2.plot(-df['bins'], df['avgWaterMass']/2, linestyle=linestyle[i], \
                 c=cm(cmap_list[i]))
        ax3.plot(df['bins'], df['avgDAMass'], linestyle=linestyle[i], \
                 c=cm(cmap_list[i]))
        ax3.plot(-df['bins'], df['avgDAMass'], linestyle=linestyle[i], \
                 c=cm(cmap_list[i]))
    else: # CNTs or BNNTs.
        df = pd.read_csv(filename, delimiter = ' ', \
                         names = ['bins', 'avgVelocity', 'vx', 'vy', \
                                  ...
                                  'avgWaterMass'], \
                         header = None)
        ax1.plot(df['bins']-diameter_dict[surface]/2, df['avgVelocity'], \
                 linestyle=linestyle[i], c=cm(cmap_list[i]))
        ax2.plot(df['bins']-diameter_dict[surface]/2, df['avgWaterMass'], \
                 linestyle=linestyle[i], c=cm(cmap_list[i]))
        ax3.plot(df['bins']-diameter_dict[surface]/2, df['avgDAMass'], \
                 linestyle=linestyle[i], c=cm(cmap_list[i]))
...
ax1.legend(legends, loc='center left', frameon=True, bbox_to_anchor=(1, 0.5))
box = ax2.get_position()
ax2.set_position([box.x0, box.y0, box.width*0.85, box.height])
ax2.legend(legends, loc='center left', frameon=True, bbox_to_anchor=(1, 0.5))
box = ax3.get_position()
ax3.set_position([box.x0, box.y0, box.width*0.85, box.height])
ax3.legend(legends, loc='center left', frameon=True, bbox_to_anchor=(1, 0.5))

fig1.savefig('../images/4_2_velocity_profile.png', dpi=300, bbox_inches='tight')
#fig2.savefig('../images/5_2_water_mass_distn.png', dpi=300, bbox_inches='tight')
fig3.savefig('../images/4_2_mass_distn_DA_zigzag.png', dpi=300, bbox_inches='tight')